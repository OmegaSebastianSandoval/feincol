...
<div class="container-fluid">
	...
	<form class="text-left" enctype="multipart/form-data" method="post" action="<?php echo $this->routeform; ?>" data-bs-toggle="validator">
		<div class="content-dashboard">
			...
			<!-- <?php print_r($padre); ?> -->
			<div class="row">
				<input type="hidden" name="contenido_padre" value="<?php if ($this->content->contenido_padre) {
				...
				<div class="col-2 form-group">
					...
					<br>
					<input type="checkbox" name="contenido_estado" value="1" class="form-control switch-form" <?php if ($this->getObjectVariable($this->content, 'contenido_estado') == 1) {
																													echo "checked";
																												} ?>></input>
						...
						<label class="input-group">
							...
							<select class="form-control" name="contenido_seccion" id="contenido_seccion" required>
								...
								<?php foreach ($this->list_contenido_seccion as $key => $value) { ?>
									<option <?php if ($this->getObjectVariable($this->content, "contenido_seccion") == $key) {
												echo "selected";
					...
					<label class="input-group">
						...
						<select class="form-control" name="contenido_columna_espacios">
							...
							<?php foreach ($this->list_contenido_columna_espacios as $key => $value) { ?>
								<option <?php if ($this->getObjectVariable($this->content, "contenido_columna_espacios") == $key) {
											echo "selected";
										} ?> value="<?php echo $key; ?>" /> <?= $value; ?></option>
							<?php } ?>
						</select>
					...
					<label class="input-group">
						...
						<select class="form-control" name="contenido_columna_alineacion">
							...
							<?php foreach ($this->list_contenido_columna_alineacion as $key => $value) { ?>
								<option <?php if ($this->getObjectVariable($this->content, "contenido_columna_alineacion") == $key) {
											echo "selected";
										} ?> value="<?php echo $key; ?>" /> <?= $value; ?></option>
							<?php } ?>
						</select>
				...
				<?php if ($tipo == 5 || $tipo == 6 || $this->contentpadre->contenido_tipo == 2) { ?>
					...
					<div class="col-2 form-group no-banner si-carrousel no-acordion si-contenido2" <?php if (isset($tipo) == false || ($tipo != 2 && $tipo != 4  && $tipo != 6)) { ?> style="display: none;" <?php } ?>>
						...
						<input type="checkbox" class="switch-form" name="contenido_borde" value="1" <?php if ($this->getObjectVariable($this->content, 'contenido_borde') == 1) {
																										echo "checked";
																									} ?>></input>
						<div class="help-block with-errors"></div>
					</div>
				...
				<div class="col-3 form-group">
					...
					<label class="input-group">
						...
						<input type="text" value="<?php if ($this->content->contenido_fecha) {
														echo $this->content->contenido_fecha;
													} else {
														echo date('Y-m-d');
													} ?>" name="contenido_fecha" id="contenido_fecha" class="form-control" data-provide="datepicker" data-date-format="yyyy-mm-dd" data-date-language="es">
					</label>
					<div class="help-block with-errors"></div>
				</div>
				...
				<div class="col-2 form-group no-banner  si-seccion no-carrousel no-acordion si-contenido2" <?php if ($tipo == 1 || $tipo == 6 || $tipo == 7  || $tipo == 0) { ?> style="display: none;" <?php } ?>>
					...
					<input type="checkbox" name="contenido_titulo_ver" value="1" class="form-control switch-form " <?php if ($this->getObjectVariable($this->content, 'contenido_titulo_ver') == 1) {
																														echo "checked";
																													} ?>></input>
					<div class="help-block with-errors"></div>
				</div>
					...
					<label class="input-group">
						...
						<select class="form-control" name="contenido_fondo_imagen_tipo">
							...
							<?php foreach ($this->list_contenido_fondo_imagen_tipo as $key => $value) { ?>
								<option <?php if ($this->getObjectVariable($this->content, "contenido_fondo_imagen_tipo") == $key) {
											echo "selected";
										} ?> value="<?php echo $key; ?>" /> <?= $value; ?></option>
							<?php } ?>
						</select>
				...
				<div class="col-4 form-group no-contenido no-banner si-seccion no-acordion si-carrousel si-contenido2" <?php if ($tipo != 2 && $tipo != 4 && $tipo != 5  && $tipo != 6  && $tipo != 22 && $tipo != 24) { ?> style="display: none;" <?php } ?>>
						...
						<div class="input-group-prepend">
							...
							<select class="form-control" name="contenido_enlace_abrir">
								<?php foreach ($this->list_contenido_enlace_abrir as $key => $value) { ?>
									<option <?php if ($this->getObjectVariable($this->content, "contenido_enlace_abrir") == $key) {
												echo "selected";
											} ?> value="<?php echo $key; ?>" /> <?= $value; ?></option>
								<?php } ?>
							</select>
